...
try:
    from src.app.explain import explain_linear_prediction
    ...
    from explain import explain_linear_prediction  # type: ignore
    from normalize import normalize_text  # type: ignore
    from prepare_ml_text import (  # type: ignore
        build_segmenter,
        is_likely_presegmented,
        load_stopwords,
        parse_ml_config,
...
# Custom exception for inference-related errors with HTTP status code support.
...
class ModelBundle:
    ...
    default_score_method: str
    mtimes: dict[str, float]


# Result object for preprocessing pipeline with diagnostics.
@dataclass
...
# Main service handling model loading, preprocessing, scoring, and prediction.
class InferenceService:

    # Initialize service with config, cache, and preprocessing pipeline.
    def __init__(self, repo_root: Path, app_config: dict[str, Any]) -> None:
        self.repo_root = repo_root
        self.app_config = app_config

        inf_cfg = app_config.get("inference", {})
        self.default_content_type = str(inf_cfg.get("default_content_type", "news")).strip().lower() or "news"
        ...
        self.include_segmentation_impact_warning = bool(
            diagnostics_cfg.get("include_segmentation_impact_warning", True)
        )
        ...
        self.cache: OrderedDict[str, ModelBundle] = OrderedDict()
    ...
    # Factory method to create service from YAML config file.
    @classmethod
    ...
    # Load preprocessing pipeline config (normalize + segment + tokenize).
    def _load_prepare_pipeline_config(self) -> None:
        ...
        try:
            ...
            requested_backend = str(self.prepare_cfg.get("segment_backend", "auto")).strip().lower()
            if requested_backend != "py_vncorenlp":
                raise

            fallback_cfg = dict(self.prepare_cfg)
            fallback_cfg["segment_backend"] = "auto"
            stage_warnings.append(
                f"segment_backend_forced_fallback_from_py_vncorenlp:{exc}"
            )
            self.segment_backend_actual, self.segment_fn = build_segmenter(fallback_cfg, stage_warnings)
            self.prepare_cfg = fallback_cfg
        ...
        self.internal_warnings = list(stage_warnings)
        for warning in self.internal_warnings:
            print(f"[APP INFERENCE INTERNAL WARNING] {warning}")


    # Preload default model bundle at startup.
    def preload_default_bundle(self) -> ModelBundle:
        return self._get_bundle(run_id=None)


    # Get file modification timestamps for cache invalidation.
    ...
    # Resolve best run_id from best_config.json.
    ...
    # Resolve artifact file paths based on run_id or fallback strategy.
    def _resolve_artifact_paths(self, run_id: str | None) -> tuple[str, Path, Path, Path]:
        ...
        if not model_path.exists() or not vectorizer_path.exists() or not metadata_path.exists():
            raise InferenceError(
                f"Missing artifacts for fallback best run_id={selected_run_id}.",
                status_code=500,
            )
        return selected_run_id, model_path, vectorizer_path, metadata_path


    # Determine default scoring method supported by model.
    def _default_score_method_for_model(self, model: Any) -> str:
        if hasattr(model, "predict_proba"):
            return "predict_proba"
        if hasattr(model, "decision_function"):
            return "decision_function_sigmoid"
        return "unsupported"


    # Resolve threshold from metadata calibration or fallback.
    ...
    # Build a ModelBundle from artifact files.
    def _build_bundle(self, run_id: str | None) -> ModelBundle:
        ...
        if text_variant not in {"text_ml_seg", "text_ml_seg_lower"}:
            raise InferenceError(
                f"Unsupported text_variant in metadata: {text_variant}. "
                "Expected one of: text_ml_seg, text_ml_seg_lower",
                status_code=500,
            )

        threshold_used = self._resolve_bundle_threshold(metadata)
        score_method = self._default_score_method_for_model(model)

        key = f"run:{selected_run_id}" if run_id else "default"
        ...
        return ModelBundle(
            ...
            default_score_method=score_method,
            mtimes=mtimes,
        )


    # Check whether cached bundle is stale based on file modification time.
    def _is_cache_entry_stale(self, bundle: ModelBundle) -> bool:
        if not self.reload_if_file_changed:
            return False
        current_mtimes = self._bundle_file_mtimes(bundle.model_path, bundle.vectorizer_path, bundle.metadata_path)
        for key in ["model", "vectorizer", "metadata"]:
            if not math.isclose(current_mtimes[key], bundle.mtimes[key], rel_tol=0.0, abs_tol=0.0):
                return True
        return False


    # Insert bundle into LRU cache with eviction.
    def _put_cache(self, key: str, bundle: ModelBundle) -> None:
    ...
    # Retrieve bundle from cache or load new one.
    def _get_bundle(self, run_id: str | None) -> ModelBundle:
    ...
    # Compute score using predict_proba or decision_function + sigmoid.
    def _compute_score(self, model: Any, x_vec: Any) -> tuple[float, float | None, str, str, bool]:
        if hasattr(model, "predict_proba"):
            ...
            score = float(probs[0, class_one_index])
            return score, None, "predict_proba", "probability", True
        ...
        if hasattr(model, "decision_function"):
            ...
            if raw_arr.ndim == 0:
                ...
                raise InferenceError("Unsupported decision_function output shape.", status_code=500)
    ...
    # Normalize, segment, tokenize, and truncate input text deterministically.
    def _normalize_and_tokenize(
        ...
        # Hard safety cap for extremely large payloads before expensive processing.
        if len(input_text) > self.max_input_chars:
            input_text = input_text[: self.max_input_chars]
            warnings.append(
                f"Input exceeded max_input_chars={self.max_input_chars}. Applied hard safety truncation."
            )
        ...
        if text_variant == "text_ml_seg":
            ...
            raise InferenceError(
                f"Unsupported text_variant='{text_variant}'. Expected text_ml_seg or text_ml_seg_lower.",
                status_code=500,
            )
        ...
        if post_normalize_token_count > self.max_input_tokens:
            ...
            warnings.append(
                f"Input tokens exceeded max_input_tokens={self.max_input_tokens}. Applied token truncation."
            )

        text_for_model = " ".join(tokens)
        if not text_for_model:
            raise InferenceError(
                "Input became empty after normalize/segment/tokenize. Please provide richer text.",
                status_code=422,
            )
        ...
        if segmentation_fallback_used and self.include_segmentation_impact_warning:
            if feature_set in {"word", "word_char"}:
                warnings.append(
                    "Segmentation fallback may degrade performance for word-based models."
                )
            else:
                warnings.append(
                    "Segmentation fallback is active; impact is usually lower for char-based models."
                )
    ...
    # Main prediction pipeline: preprocess → vectorize → score → explain.
    def predict(
        self,
        text: str,
        run_id: str | None,
        ...
        resolved_content_type = (content_type or self.default_content_type).strip().lower()
        ...
        prep = self._normalize_and_tokenize(
            text=text,
        ...
        score, raw_decision_score, score_method, confidence_type, is_probability = self._compute_score(
            bundle.model,
            x_vec,
        )
    ...
    # Health check endpoint returning service and cache status.
    def health(self) -> dict[str, Any]:
        ...
        return {
            ...
            "default_score_method": default_bundle.default_score_method if default_bundle else None,
