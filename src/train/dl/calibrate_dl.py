...
try:
    from src.train.dl.train_phobert import get_repo_root, load_yaml
...
# Parse CLI args for DL threshold calibration workflow.
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Calibrate DL decision threshold and update run metadata.")
    parser.add_argument(
        "--run-id",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--output-report",
        type=str,
        default="reports/threshold_calibration_dl.md",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional figure path. Default: reports/figures/threshold_curve_dl_<run_id>.png",
    )
    parser.add_argument(
        "--config-path",
        type=str,
...
def load_json(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise CalibrationError(f"Missing JSON artifact: {path}")
    with path.open("r", encoding="utf-8") as f:
        payload = json.load(f)
    if not isinstance(payload, dict):
        raise CalibrationError(f"JSON artifact must be an object: {path}")
    return payload


# Load DL best_config.json with strict object validation.
def load_best_config(best_config_path: Path) -> dict[str, Any]:
    return load_json(best_config_path)
...
def resolve_run_id(run_id: str | None, repo_root: Path) -> str:
    ...
    best_config = load_best_config(repo_root / "experiments" / "dl" / "best_config.json")
...
def load_runs_rows(runs_path: Path) -> list[dict[str, Any]]:
    if not runs_path.exists():
        raise CalibrationError(f"Missing runs file: {runs_path}")
    with runs_path.open("r", encoding="utf-8-sig", newline="") as f:
        reader = csv.DictReader(f)
        rows = list(reader)
    if not rows:
        raise CalibrationError("experiments/dl/runs.csv has no run records.")
    return rows


# Resolve one run row by run_id for robust artifact path resolution.
def resolve_run_row(run_rows: list[dict[str, Any]], run_id: str) -> dict[str, Any]:
    requested = run_id.strip()
    for row in run_rows:
        if str(row.get("run_id", "")).strip() == requested:
            return row
    raise CalibrationError(f"run_id '{requested}' not found in experiments/dl/runs.csv.")


# Resolve physical run directory using run notes or fallback naming convention.
def resolve_run_dir(run_row: dict[str, Any], repo_root: Path) -> Path:
    run_id = str(run_row.get("run_id", "")).strip()
    if not run_id:
        raise CalibrationError("Target run row is missing run_id.")
    ...
    if not fallback_run_dir.exists():
        raise CalibrationError(f"Run directory not found: {fallback_run_dir}")
    return fallback_run_dir


# Resolve mandatory predictions/metadata artifacts for one run.
def resolve_artifact_paths(run_dir: Path) -> tuple[Path, Path]:
    predictions_path = run_dir / "predictions.csv"
    metadata_path = run_dir / "metadata.json"

    if not predictions_path.exists():
        raise CalibrationError(f"Predictions artifact not found: {predictions_path}")
...
def load_predictions(predictions_path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise CalibrationError(f"predictions.csv missing required columns: {missing}")
    return df
...
# Extract y_true and prob_fake scores for one split with strict validation.
def extract_scores_by_split(df_pred: pd.DataFrame, split_name: str) -> tuple[np.ndarray, np.ndarray]:
    split_rows = df_pred[df_pred["split"].astype(str).str.lower() == split_name].copy()
    if split_rows.empty:
        raise CalibrationError(f"No prediction rows found for split='{split_name}'.")

    y_true = pd.to_numeric(split_rows["y_true"], errors="coerce")
    scores = pd.to_numeric(split_rows["prob_fake"], errors="coerce")
    if y_true.isna().any() or scores.isna().any():
        raise CalibrationError(f"{split_name}: y_true/prob_fake contains invalid values.")

    y_true_np = y_true.astype(int).to_numpy()
    if not set(np.unique(y_true_np)).issubset({0, 1}):
        raise CalibrationError(f"{split_name}: y_true contains values outside {{0,1}}.")

    scores_np = np.clip(scores.astype(float).to_numpy(), 0.0, 1.0)
...
def build_report_markdown(
    ...
    lines: list[str] = []
    lines.append("# DL Threshold Calibration Report")
    lines.append("")
    ...
    lines.append("- default_threshold: 0.500000")
...
def run_calibration(args: argparse.Namespace) -> tuple[str, float, Path, Path]:
    ...
    print("[DL CALIBRATE] Starting threshold calibration...")
    ...
    resolve_split_path(config, args.split_calibration, repo_root)
    resolve_split_path(config, args.split_eval, repo_root)

    run_rows = load_runs_rows(repo_root / "experiments" / "dl" / "runs.csv")
    run_row = resolve_run_row(run_rows, run_id)
    run_dir = resolve_run_dir(run_row, repo_root)

    predictions_path, metadata_path = resolve_artifact_paths(run_dir)
    predictions = load_predictions(predictions_path)
    metadata = load_metadata(metadata_path)

    y_calibration, calibration_scores = extract_scores_by_split(predictions, args.split_calibration)
    y_eval, eval_scores = extract_scores_by_split(predictions, args.split_eval)
    ...
    if args.output_figure:
        ...
        figure_path = repo_root / "reports" / "figures" / f"threshold_curve_dl_{run_id}.png"
    ...
    print("[DL CALIBRATE] PASS")
    print(f"[DL CALIBRATE] run_id={run_id}")
    print(f"[DL CALIBRATE] score_method={score_method}")
    print(
        "[DL CALIBRATE] thresholds "
    ...
    print(f"[DL CALIBRATE] metadata={metadata_path.as_posix()}")
    print(f"[DL CALIBRATE] report={report_path.as_posix()}")
    print(f"[DL CALIBRATE] figure={figure_path.as_posix()}")
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"[DL CALIBRATE ERROR] {exc}")
